...
assert.equal(hote.state.cards["01105"].loc.pile, "removed");
d = await hote.action({ t: "advanceAct" });
assert.equal(hote.state.cards[hote.state.actId].code, "01109");
d = await hote.action({ t: "setSeatCounter", seat: 0, key: "clues", delta: 3 });
